  ...
  describe 'GET /institutions/get_all_institutions' do
    ...
    it 'returns the correct number of institutions when page_size is specified' do
      (1..20).each do |page_size|
        get '/institutions/get_all_institutions', params: { page_size: page_size }
        expect(JSON.parse(response.body).size).to equal(page_size)
      end
    end

    it "doesn't allow negative page_size" do
      get '/institutions/get_all_institutions', params: { page_size: -1 }
      expect(response.status).to eq(422)
    end

    it "doesn't allow 0 as page_size" do
      get '/institutions/get_all_institutions', params: { page_size: 0 }
      expect(response.status).to eq(422)
    end
